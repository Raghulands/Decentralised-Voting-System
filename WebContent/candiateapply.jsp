                ...
                <tbody>
                    <%
                        try {
                            ...
                            PreparedStatement ps = con.prepareStatement("SELECT * FROM nominatecandidate WHERE email=?");
                            ...
                            while(rs.next()){
                                ...
                                byte[] img = rs.getBytes("student_id_photo");
                    %>
                    <tr>
                        ...
                        <td>
                            <% if(img != null){ %>
                                <img src="data:image/jpeg;base64,<%=Base64.getEncoder().encodeToString(img)%>" class="candidate-image"/>
                            <% } else { %>
                                No Image
                            <% } %>
                        </td>
                       <td>
                                <a href="candidateapp.jsp?email=<%=rs.getString(3)%>&ass=<%=rs.getString(2)%>&dis=<%=rs.getString(1)%>&mobile=<%=rs.getString(4)%>" 
                                   class="btn-form">
                    ...
                    <% }
                        if(!hasApplications){ %>
                    <tr>
                        <td colspan="5" class="no-applications"><i class="fas fa-inbox"></i> No applications yet</td>
                    </tr>
                    <% }
